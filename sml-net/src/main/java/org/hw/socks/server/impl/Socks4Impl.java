...
public class Socks4Impl implements SocksCommonInterface {
	...
	{
		LoggerHelper.getLogger().debug(getClass(),"Socks 4 reply: \""+replyName( ReplyCode)+"\"" );
	...
	protected	void	refuseCommand( byte errorCode )	{
		LoggerHelper.getLogger().debug(getClass(),"Socks 4 - Refuse Command: \""+replyName(errorCode)+"\"" );
	...
	public void	connect() throws Exception {
		...
		LoggerHelper.getLogger().debug(getClass(), "Connecting..." );
	//	Connect to the Remote Host
		...
		LoggerHelper.getLogger().debug(getClass(), "Connected to "+DebugLog.getInstance().getSocketInfo( m_Parent.m_ServerSocket ) );
	...
	public	void	bindReply( byte ReplyCode, InetAddress IA, int PT ) throws	IOException {
		...
		if( m_Parent.isActive() )	{
			...
			LoggerHelper.getLogger().debug(getClass(), "Closed BIND Client Connection" );
		}
	} // Reply_Command()
	...
	public	InetAddress	resolveExternalLocalIP()	{
		...
		if( m_ExtLocalIP != null )	{
			...
			try	{
				...
				LoggerHelper.getLogger().debug(getClass(), "WARNING !!! THE LOCAL IP ADDRESS WAS CHANGED !" );
			}
		}
		...
		for( int i=0;i<hosts.length;i++ )	{
			try	{
				...
				LoggerHelper.getLogger().debug(getClass(),"Error in BIND() - BIND reip Failed at "+i );
			}
		}
	...
	public void	bind()	throws IOException
	{
		...
		LoggerHelper.getLogger().debug(getClass(), "Binding..." );
		// Resolve External IP
		MyIP = resolveExternalLocalIP();

		LoggerHelper.getLogger().debug(getClass(),"Local IP : " + MyIP.toString() );
		...
		try	{	
			...
			LoggerHelper.getLogger().error(getClass(),"Error in BIND() - Can't BIND at any Port" );
			bindReply( (byte)92, MyIP,MyPort );
			ssock.close();
			return;
		}

		LoggerHelper.getLogger().debug(getClass(), "BIND at : <"+MyIP.toString()+":"+MyPort+">" );
		...
		while( socket == null )
		{
			if( m_Parent.checkClientData() >= 0 ) {
				LoggerHelper.getLogger().debug(getClass(), "BIND - Client connection closed" );
				ssock.close();
				return;
			}
...
*/		
	...
	{
		LoggerHelper.getLogger().debug(getClass(),"Error - Socks 4 don't support UDP Association!" );
		LoggerHelper.getLogger().debug(getClass(), "Check your Software please..." );
