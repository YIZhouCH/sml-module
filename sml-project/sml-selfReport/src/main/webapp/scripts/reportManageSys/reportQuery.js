...
var reportQuery = {
	...
	appendDiv: function() {
		var divS = '<div style="cursor:pointer;display:inline-block;margin-right: 30px;float: right;" onClick="reportQuery.loadTree()">' +
			'<img height="20px" width="25px" src="../../static/interfaceManager/images/link.jpg" />' +
			'</div>'
		...
		$('.panel-title').html(menueTitle+divS);
	},
	zhuanhuaunTreeData: function(data) {
		...
		if (data.length > 0) {
			for (var i = 0; i < data.length; i++) {
				...
				if (data[i].type == 2) {   //jsp
					object.iconCls = "icon-wenben"; 
					object.attributes = {
						"ishasChilds": false,
						"parent_id": data[i].parent_id,
						"type":data[i].type
				...
				$('#tree').tree({
					...
					onContextMenu: function(e, node) {
						...
						if (!node.attributes.ishasChilds) {
							if (node.iconCls == "icon-wenbenfabu" || node.iconCls == "icon-wenben") {
								$('#mmm').menu('show', {
									left: e.pageX,
									top: e.pageY
								});
