...
<script type="text/javascript">
    $(function() {
    	var resource=parent.$.modalDialog.node;
    	var formLst=$.stringToList('id,name,urls,open_mode@select,description,icon,pid,seq,status@select,opened@select,resource_type@select,create_time');
    	for(var i=0;i<formLst.length;i++){
    		var key=formLst[i].split('@');
    		if(key.length==1){
    			$("input[name='"+key+"']").val(resource[key]);
    		}else{
    			$(key[1]+"[name='"+key[0]+"']" ).val(resource[key[0]]);
    		}
    	}
        ...
        $('#resourceEditForm').form({
            url : smlManager.getUpUrl('system-resource-up','update'),
            onSubmit : function() {
                progressLoad();
      ...
      <input name="upType" type="hidden"  value="update" >
        <table  class="grid">
            <tr>
                ...
                <td>资源类型</td>
                <td>
                    <select id="resourceEditType" name="resource_type" class="easyui-combobox" data-options="width:140,height:29,editable:false,panelHeight:'auto'">
            ...
            <tr>
                <td>资源路径</td>
                <td><input name="urls" type="text"  placeholder="请输入资源路径" class="easyui-validatebox span2" ></td>
                <td>打开方式</td>
                <td>
                    <select id="resourceEditOpenMode" name="open_mode" class="easyui-combobox" data-options="width:140,height:29,editable:false,panelHeight:'auto'">
